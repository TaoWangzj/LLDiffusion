...
class VELOLDataset(Dataset):
    def __init__(self, dir, patch_size, n, transforms, filelist=None, parse_patches=True, augment=None, training=True):
        super().__init__()
        if filelist is None:
            LOL_dir = dir
            ...
            # low train filelist
            if training:
                lr_root = os.path.join(LOL_dir, 'VE-LOL-L-Cap-Low_train')
                gt_root = os.path.join(LOL_dir, 'VE-LOL-L-Cap-Normal_train')
            else:
                lr_root = os.path.join(LOL_dir, 'VE-LOL-L-Cap-Low_test')
                gt_root = os.path.join(LOL_dir, 'VE-LOL-L-Cap-Normal_test')
            ...
            images = sorted(os.listdir(gt_root))
            images = [image for image in images if image.endswith(".png")]
    ...
    def get_image(self, index):
        ...
        int_string = stem.lstrip(ascii_letters)
        img_id = int(int_string)
...
class VELOL:
    ...
    def train_loader(self, training=True):
        ...
        train_loader = DataLoader(
            ...
            pin_memory=True,
            collate_fn=VELOLDataset.collate_fn,
        )
        return train_loader

    def val_loader(self, training=True, num_vals=20):
        root = self.config.data.root

        val_dataset = VELOLDataset(
            dir=root,
            n=self.config.training.patch_n,
            patch_size=256,  # 256 for validation
            transforms=self.transforms,
            parse_patches=False,
            training=training,
        )
        val_dataset = Subset(val_dataset, list(range(num_vals)))
